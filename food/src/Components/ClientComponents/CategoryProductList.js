//code of add cart product in localstorage with login condition
...
// const {proname,category,mrp,discountmrp,stock,status,descriptionimage} = product;
...
// 	if ( getScrollTop() < getDocumentHeight() + 3000 - window.innerHeight) return;
...
//    const productExist = wish.find((item)=>
//    item._id === pro._id);
// //    console.log("wish me");
// //    console.log(productExist);
//    if(productExist){

// 	   setWish(wish.map((item)=>item._id === pro._id

// 	   ?  {...productExist,qty:productExist.qty}:item
...
// // function add product in cart and local storage
// const addtocat = (product,id)=>{
...
// 										<div className="item" data-value="2">Price - High to Low</div>
// 										<div className="item" data-value="3">Alphabetical</div>
// 										<div className="item" data-value="4">Saving - High to Low</div>
...
// 							<Grid item xxl={2}  xl={3}   key={pro.id} className={classes.box}>
// <div>
// 								<Link to={`/viewsingleproduct/${pro._id}`} className="product-img">

//                                      <img src={ "http://127.0.0.1:9002/public/product/"+pro.image} alt=""/>
// 									 <div className="product-absolute-options">
// 										<span className="offer-badge-1">{pro.discountmrp}% off</span>

// 									</div>
// 								</Link>

// 								<div className="product-text-dt">
// 									<p>{pro.status}<span>(Stock{pro.stock})</span></p>
// 									<h4>{pro.proname}</h4>
// 									<div className="product-price">${pro.mrp}<span></span></div>
// 									<div className="qty-cart">

// 									<span className="like-icon save-icon" onClick={()=>addtoWishlist(pro)} title="wishlist"></span>
// 									<span className="cart-icon"><i  onClick={()=>addtocat(pro)} className="uil uil-shopping-cart-alt"></i></span>
// 									</div>
// 								</div>
// 								</div>
// 								</Grid>
...
//code of add cart product in direct databise with login condition
...
const Demo = () => {
  ...
  const addtocat = async (pro, id) => {
    ...
    if (customerr) {
      ...
      //logic of add unique id and incrise when add buttern more than one times
